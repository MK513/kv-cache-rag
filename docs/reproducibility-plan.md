...
> **결론 (2026-09-22): 재현성 목표를 접었다.** 아래 조사와 Phase 1 적용은 그대로
> 유효하지만, "같은 보고서가 다시 나온다" 는 목표는 포기했다. 이유는 맨 아래 「실측 결과」와
> 「왜 접었나」 참고. 현재 정책은 README 「재현성 — 어디까지 되고 어디부터 안 되는가」 다.

원래 목표 — **같은 코퍼스로 다시 돌리면 같은 주장이 나오고, 검토 판정이 그대로 이월된다.**
...
응답 캐시까지 넣고 나서 "클론한 사람이 한 번 실행하면 PDF 가 나오는가" 를 따져 보니
구조적으로 불가능했다.

| 막는 것 | 해결에 필요한 것 |
|---|---|
| `data/raw/` 가 Git 제외 (15MB) | 원문 커밋 — 재배포 판단 필요 |
| 웹 출처 4건의 원본이 바뀐다 | 위와 동일 |
| `.cache/llm.sqlite` 가 Git 제외 | 캐시 공유 |
| **이해관계자 노드가 매 실행 웹을 새로 검색한다** | **웹 재생 (Phase 3)** |
| `libpango` · API 키 | 환경 설정 |

**마지막 항목이 관문이다.** Tavily 결과가 매번 달라 stakeholder 프롬프트가 달라지고,
캐시가 적중하지 않아 새 Claim 이 생기며, 원장에 없는 지문이라 `review_status=pending` 으로
멈춘다. 즉 **웹 재생 없이는 "클론 → 실행 → PDF" 가 성립하지 않는다.**

Phase 3 은 설계서 §7 *노드 실행 전에는 해당 실행의 자료만 사용하며* 와
`docs/interface.md:298` *다른 실행의 캐시로 대체하지 않는다* 를 어긴다. 계약 개정과 R3
승인이 선행돼야 한다.

그리고 애초에 **이 파이프라인은 사람의 내용 검토를 필수로 설계됐다**(§8). 원샷 재현은 그
설계와 상충한다. 검증이 목적이라면 재실행이 아니라 `runs/<run_id>/` 산출물을 보는 쪽이 맞다.

**유지하는 것** — Phase 1(임베딩 revision 핀, 코퍼스 해시 검사)과 응답 캐시, 검토 판정
원장은 그대로 둔다. 재현성 주장이 아니라 **같은 기계에서 재검토를 줄이고 조용한 드리프트를
막는** 장치로서 값어치가 있다.
